...
DROP DATABASE IF EXISTS securityservice;
DROP USER IF EXISTS `security_service`@`%`;
CREATE DATABASE IF NOT EXISTS securityservice CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER IF NOT EXISTS `security_service`@`%` IDENTIFIED WITH mysql_native_password BY 'password';
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, REFERENCES, INDEX, ALTER, EXECUTE, CREATE VIEW, SHOW VIEW,
CREATE ROUTINE, ALTER ROUTINE, EVENT, TRIGGER ON `securityservice`.* TO `security_service`@`%`;
FLUSH PRIVILEGES;
